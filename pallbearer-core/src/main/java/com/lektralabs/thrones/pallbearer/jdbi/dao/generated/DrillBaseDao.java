...
public interface DrillBaseDao {

    @RegisterBeanMapper(DrillRow.class)
    @UseStringTemplateSqlLocator
    @SqlQuery("select")
    Optional<DrillRow> findById(UUID id);

    @RegisterBeanMapper(DrillRow.class)
    @UseStringTemplateSqlLocator
    @SqlQuery("selectAll")
    List<DrillRow> findAll(@Define("findOptions") String findOptions);

    @UseStringTemplateSqlLocator
    @SqlUpdate("insert")
    @GetGeneratedKeys("id")
    UUID insert(@BindBean DrillRow item);

    @UseStringTemplateSqlLocator
    @SqlUpdate("update")
    int update(@BindBean DrillRow item, Integer currentVersion);
