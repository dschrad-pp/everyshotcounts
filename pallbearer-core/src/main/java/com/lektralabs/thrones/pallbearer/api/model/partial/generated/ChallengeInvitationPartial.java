...
@NoArgsConstructor
@AllArgsConstructor
public class ChallengeInvitationPartial implements Serializable, EntityMethods {

    private Optional<UUID> challengeInvitationId;
    private UUID challengeId;
    private UUID inviteUserId;
    private UUID tokenId;
    private Optional<Integer> version;

    public ChallengeInvitationRow toRow(CurrentUser auditUser) {
        long now = DateTimeUtils.now().getMillis();
        return  ChallengeInvitationRow.builder()
                .id(optionalFactory(challengeInvitationId, UUID.randomUUID()))
